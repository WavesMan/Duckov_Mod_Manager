...
class WebSocketMessage {
  ...
  final OperationPriority priority; // 消息优先级
  ...
  WebSocketMessage({
    ...
    DateTime? timestamp,
    this.metadata,
    this.priority = OperationPriority.system, // 默认为系统优先级
  })  : type = type,
...
/// 异步WebSocket客户端 - 支持操作优先级队列和防止重复连接
class AsyncWebSocketClient {
  ...
  // 消息队列 - 分为用户操作队列和系统操作队列
  final List<WebSocketMessage> _userMessageQueue = [];
  final List<WebSocketMessage> _systemMessageQueue = [];
  ...
  /// 发送高优先级消息（用户操作）
  Future<WebSocketMessage> sendUserMessage({
    ...
    final message = WebSocketMessage(
      ...
      data: data,
      metadata: metadata,
      priority: OperationPriority.user, // 用户操作优先级
    );

    return await _sendMessageWithPriority(message, timeout ?? config.requestTimeout);
  }
  ...
  Future<WebSocketMessage> sendMessage({
    ...
    final message = WebSocketMessage(
      ...
      data: data,
      metadata: metadata,
      priority: OperationPriority.system, // 系统操作优先级
    );
  ...
  /// 发送带优先级的消息
  Future<WebSocketMessage> _sendMessageWithPriority(WebSocketMessage message, Duration timeout) async {
    // 如果未连接，加入队列
    if (!isConnected) {
      _log('WebSocket未连接，消息加入队列: ${message.action} (优先级: ${message.priority.name})');
      
      // 根据优先级加入不同的队列
      if (message.priority == OperationPriority.user) {
        _userMessageQueue.add(message);
      } else {
        _systemMessageQueue.add(message);
      }
    ...
    return await _sendMessageInternal(message, timeout);
  }
  
  /// 入队用户消息
  void queueUserMessage(WebSocketMessage message) {
    _userMessageQueue.add(message.copyWith(priority: OperationPriority.user));
    _log('用户消息已加入队列: ${message.action} (ID: ${message.id})');
  }
  
  /// 入队系统消息
  void queueSystemMessage(WebSocketMessage message) {
    _systemMessageQueue.add(message.copyWith(priority: OperationPriority.system));
    _log('系统消息已加入队列: ${message.action} (ID: ${message.id})');
  }
  
  /// 队列长度
  int get messageQueueLength => _userMessageQueue.length + _systemMessageQueue.length;
  int get userMessageQueueLength => _userMessageQueue.length;
  int get systemMessageQueueLength => _systemMessageQueue.length;
  ...
  Future<WebSocketMessage> _sendMessageInternal(WebSocketMessage message, Duration timeout) async {
    try {
      ...
      _log('发送消息: ${message.action} (ID: ${message.id}, 优先级: ${message.priority.name})');
  ...
  void _processMessageQueue() async {
    if (_isProcessingQueue || (_userMessageQueue.isEmpty && _systemMessageQueue.isEmpty)) return;
    
    _isProcessingQueue = true;
    
    try {
      // 优先处理所有用户操作
      while (_userMessageQueue.isNotEmpty && isConnected) {
        final message = _userMessageQueue.removeAt(0);
        if (_channel == null && config.offlineMode) {
          ...
          try {
            ...
            _log('用户队列消息发送失败: $e');
            // 如果发送失败，重新加入队列开头
            _userMessageQueue.insert(0, message);
            break;
          }
        }
      }
      
      // 处理系统操作（只有在没有用户操作时才处理）
      if (_userMessageQueue.isEmpty) {
        while (_systemMessageQueue.isNotEmpty && isConnected) {
          final message = _systemMessageQueue.removeAt(0);
          if (_channel == null && config.offlineMode) {
            // 离线模式下直接丢弃或标记为已处理
            _log('离线模式，跳过发送队列消息: ${message.action}');
            continue;
            ...
            try {
              ...
              _log('系统队列消息发送失败: $e');
              // 如果发送失败，重新加入队列开头
              _systemMessageQueue.insert(0, message);
              break;
            }
          }
        }
  ...
  void _log(String message, {Map<String, dynamic>? metadata}) {
    final logMessage = '[AsyncWebSocketClient] $message';
    print(logMessage);
